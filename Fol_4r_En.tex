...
\textbf{Moses} first mentions the celestial bodies\footnote{`celestium'
  and `himlischen ding'.} that God placed in the firmament so that they
might shine in the heaven\footnote{`celo' and `himel'.} and illuminate
the \emph{earth}, namely the sun, moon, and stars\footnote{`stellas' and
  `stern'.} with which the upper\footnote{`superior' and `oberteil'.}
...
remained for him to speak about the workings of the stars\footnote{`siderum'
  and `gestirns'.} and their function, to explain for what purpose they
were created and for what task God assigned them.\footnote{`Celestium
  enim corporum' and `himlischen leiplichen ding'.}
...
entire world\footnote{`mundi totius' and `ga{[}n{]}tze{[}n{]} werlt'.},
in which heaven and ether rotate in a perfect circle through the entire
space of the universe\footnote{`vniuersi' and `werlt'.} in 24 hours. The
other is the actual, multifaceted, and varied movement of (ii) the
celestial bodies\footnote{`siderum' and `gestirns'.}, with the main
...
\emph{illumination}\footnote{`illuminatio' and `erlewchtu{[}n{]}g'.}
(II), when he said that they were arranged to shine in the
heaven\footnote{`vt lucerent in celo' and `zu scheine{[}n{]} am{[}m{]}
  hymel'.} and to light up the earth. Thus, the bodies of the moon, the
sun and the stars\footnote{`stellarum' and `stern{[}n{]}'.} are
distributed\footnote{`distributa' and `außgetailt'.} for this
purpose\footnote{`ministeria' and `dienstperkeiten'.}. Although the sun
...
\emph{brilliance}\footnote{`fulgore clarissimo' and `allerklarste{[}m{]}
...
\emph{`glory'}\footnote{`humor' and `feüchtigkeit' (to dispel the
...
animals\footnote{`a{[}n{]}{[}m{]}alia'(sic) and `geschöpff'.} in their
